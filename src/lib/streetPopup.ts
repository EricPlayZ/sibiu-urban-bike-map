import { featureHasBikeLane, featureHasIllegalParking, featureHasReservedParking, LAYER_COLORS } from "./space";
...
export function streetPopupHtml(props: Record<string, unknown>, schools: GeoJSON.FeatureCollection | null) {
    ...
    const media = photo ? `<div class="mp-media"><img src="${escapeHtml(photo)}" alt="" loading="lazy" /></div>` : `<div class="mp-media mp-media-empty" aria-hidden="true"><span class="mp-media-ico">${ICONS.photo}</span><span>Foto stradă — în curând</span></div>`;

    const rows: string[] = [];
    rows.push(row(bike ? "bike" : "off", "bike", "Pistă de biciclete", bike ? "Da" : "Nu"));
    ...
    if (!bike && !illegal && !reserved && !arondat) {
        rows.push(row("muted", "empty", "Date stradă", "Nu există date specifice"));
    }

    const chips: string[] = [];
    if (bike) chips.push(`<span class="mp-chip" style="--c:${LAYER_COLORS.bike}">Pistă</span>`);
...
export function assignFlagOffsets(p: Record<string, unknown>, opts: { includeSchool: boolean; includeEdit?: boolean }) {
    const keys: ("bike" | "rsv" | "ill" | "sch" | "edit")[] = [];
    if (p.bike_lane) keys.push("bike");
    if (p.rsrvd_park) keys.push("rsv");
    if (p.illgl_park) keys.push("ill");
...
export function schoolPopupHtml(props: Record<string, unknown>, streets: GeoJSON.FeatureCollection | null, neighborhoods: Set<string>) {
    ...
    const bike = assigned.filter((f) => featureHasBikeLane(f.properties as Record<string, unknown>)).length;
    const illegal = assigned.filter((f) => featureHasIllegalParking(f.properties as Record<string, unknown>)).length;

    const media = photo ? `<div class="mp-media"><img src="${escapeHtml(photo)}" alt="" loading="lazy" /></div>` : `<div class="mp-media mp-media-empty" aria-hidden="true"><span class="mp-media-ico">${ICONS.photo}</span><span>Foto școală — în curând</span></div>`;
